...
public class MySecurityConfig {
	...
	public FilterRegistrationBean<CorsFilter> corsFilter() {
		...
		CorsConfiguration corsConfiguration = new CorsConfiguration();
		corsConfiguration.setAllowCredentials(true);
		corsConfiguration.addAllowedOriginPattern("*"); // Allow any origin
		corsConfiguration.addAllowedHeader("Authorization");
		corsConfiguration.addAllowedHeader("Content-Type");
		corsConfiguration.addAllowedHeader("Accept");
		...
		// Set max age for caching the CORS response
		corsConfiguration.setMaxAge(3600L);

		// Setup the CorsConfigurationSource
		UrlBasedCorsConfigurationSource source = new UrlBasedCorsConfigurationSource();
		source.registerCorsConfiguration("/**", corsConfiguration);

		// Register the filter with the source
		FilterRegistrationBean<CorsFilter> bean = new FilterRegistrationBean<>(
				new CorsFilter((CorsConfigurationSource) source));
		bean.setOrder(-110); // Set filter order, -110 ensures it runs before Spring Security filters
		return bean;
	}
}
